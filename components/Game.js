...
function val(r){if(r==='A')return 11;if(['K','Q','J'].includes(r))return 10;return parseInt(r,10);}
...
export default function Game() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      // формируем initData, даже если на десктопе есть только initDataUnsafe
      let initData = tg?.initData || '';
      if(!initData && tg?.initDataUnsafe){
        const p=new URLSearchParams();
        Object.entries(tg.initDataUnsafe).forEach(([k,v])=>{
  ...
  const dbl=()=>{
    if(st!=='playing')return;
    const w=parseInt(bet,10); if(w*2>(user?.coins??0)){setMsg('Недостаточно монет');return;}
    setBet(String(w*2)); const p=[...pl,draw()]; setPl(p);
